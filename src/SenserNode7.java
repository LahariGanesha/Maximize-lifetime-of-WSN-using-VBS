...
public class SenserNode7 extends Frame implements ActionListener {
    	...
    	public void run() {
    		...
    		try {
    			...
    			while (true) {
    				...
    				tl3.setText("Passing from Node-3 to Sink");
    	    		try {      				
    	    			Thread.sleep(1500);      	    		
        	    	} 	    		
        	    	catch(Exception e) {
        	    	...
        	    	try{
    	    			String message;
    	    			requestSocket = new Socket(sinkserver_ip, 1117);
    	    			System.out.println("Connected to localhost in port 1117");
    	    			//2. get Input and Output streams
    	    			out = new ObjectOutputStream(requestSocket.getOutputStream());
    	    			out.flush();
    	    			message =  strLine + "Node-7)";  	    			
...
//    				infoText.setText(strLine);
    				flag = !flag;
    				if (flag){
    					SetStatus("Node 7 - Active");
    				} else {
       					SetStatus("Node 7 - Sleeping");
    				}
...
// add to send-info
    		try{
    			...
    			out = new ObjectOutputStream(requestSocket.getOutputStream());
    			out.flush();
    			message = infoText.getText()+ "(Path:Node-7)";
...
//        toolbarPanel.setLayout(new () );
    ...
    //    toolbarPanel.add(sourceNode);
        ...
        Thread t1 = new Thread (new PortListener(2117));
		t1.setName("Togel node-7");
		t1.start();
		
		Thread t2 = new Thread (new PortListener2(3113));
		t2.setName("lision node-3");
		t2.start();
    ...
    public static void main(String[] args) {
    	...
    	SenserNode7 mainFrame = new SenserNode7();
